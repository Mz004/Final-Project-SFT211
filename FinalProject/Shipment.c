...
/*BUG 1*/	//Edit: Manav Zadafiya, update below condition to match destination
		else if ((destination.row < min || destination.row > max) || (destination.col < min || destination.row > max))
		{
			check = -3;
			printf("Invalid destination\n");
		}  //Add: Manav Zadafiya, Add below condition to double validate the destination within map
